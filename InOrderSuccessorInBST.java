...
public class InOrderSuccessorInBST {
  public static void main(String[] args) {
		/*
		 * Node tree = new Node(20); tree.left = new Node(9); tree.left.parent =
		 * tree; tree.left.left = new Node(5); tree.left.left.parent =
		 * tree.left; tree.left.right = new Node(12); tree.left.right.parent =
		 * tree.left; tree.left.right.left = new Node(11);
		 * tree.left.right.left.parent = tree.left.right; tree.left.right.right
		 * = new Node(14); tree.left.right.right.parent = tree.left.right;
		 ...
		 * "In order successor = " + result.data);
		 */
		...
		/*
		 * Node root = new Node(5); root.left = new Node(3); root.left.parent =
		 * root; root.left.left = new Node(1); root.left.left.parent =
		 * root.left; root.right = new Node(7); root.right.parent = root;
		 */
		Node result = findInOrderSuccessor(root.left.right);
		System.out.println(result == null ? "Not found" : "Inorder successor with parent pointer = " + result.data);

		Node result2 = findInOrderSuccessorWithNoParentPointer(root,root.left.right);
		System.out.println(result == null ? "Not found" : "Inorder successor with NO parent pointer = " + result2.data);
	}
	/*
	 * Solution : When a pointer to parent is given 0. check of node is null,
	 * then return null. 1. Check for right child, if found return the minimum
	 * node of right subtree. 2. If right child is null, return the parent whos
	 * value is greater than given node. Runtime : O(H), Space : O(1)
	 */
	...
	/*
	 * Solution : When NO parent pointer available 0. check of node is null,
	 * then return null. 1. Check for right child, if found return the minimum
	 * node of right subtree. 2. If right child is null, search from root as
	 * similar to binary search and find the parent Runtime : O(H), Space : O(1)
	 */
	private static Node findInOrderSuccessorWithNoParentPointer(Node root,
			Node node) {
		if (node == null)
			return null;
		if (node.right != null) { // case 1 when right != null, get the minimum on right subtree
