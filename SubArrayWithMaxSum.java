...
public class SubArrayWithMaxSum {
	 ...
	 * QUESTIONS TO ASK
	 * 
	 * 1. What elements are stored in the array either integer or double or float 
	 * 2. is the sum of all numbers within the range of integers ? or should I use long 
	 * 3. what if the input array is null or empty? 
	 * 4. Can we use any Java built in library methods?
	 */
	/**
	...
	public static void subArrayMaxSumSlow(int[] nums) {
		...
		for (int i = 0; i < nums.length; i++) {
			...
			for (int j = i; j < nums.length; j++) {
				...
				if (currSum > maxSum) {
					maxSum = currSum;
					startIndex = i; //update the start and end indices when the max sum changes
					endIndex = j;
				}
			}
		}
