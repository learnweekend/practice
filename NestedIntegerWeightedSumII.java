 ...
 * Example 2:
 * 
 * Input: [1,[4,[6]]] Output: 17 Explanation: One 1 at depth 3, one 4 at depth 2, and one 6 at depth
 * 1; 1*3 + 4*2 + 6*1 = 17.
 *
 */
 ...
 */
public class NestedIntegerWeightedSumII {
	...
	private int depthSumInverseHelper(List<NestedInteger> nestedList, int depth) {
		...
		for (NestedInteger value : nestedList) {
			if (value.isInteger()) {
				weightedSum += value.getInteger() * depth;
			} else {
				weightedSum += depthSumInverseHelper(value.getList(), depth - 1);
			}
		}
		return weightedSum;
	}

	private static int getMaxDepth(List<NestedInteger> nestedList) {
		if (nestedList == null || nestedList.size() == 0)
			return 0;
		...
		for (NestedInteger value : nestedList) {
			if (!value.isInteger()) {
				depth = Math.max(depth, getMaxDepth(value.getList())); // Note : Get the max
			}
		}
		return depth + 1;
	}
