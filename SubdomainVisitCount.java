 ...
 * 
 * Example 1: Input: ["9001 discuss.leetcode.com"] Output: ["9001 discuss.leetcode.com", "9001
...
public class SubdomainVisitCount {
	...
	// Runtime : O(N) * max length of domains
	// Space : O(N) - for the Map
	public static List<String> subdomainVisits(String[] cpdomains) {
		List<String> visitedCounts = new ArrayList<>();
		...
		for (String domain : cpdomains) {
			String[] domainSplit = domain.split("\\s+");
			int count = Integer.parseInt(domainSplit[0]);
			String[] domains = domainSplit[1].split("\\.");
			String key = "";
			for (int i = domains.length - 1; i >= 0; i--) {
				key = (i == domains.length - 1) ? domains[i] : domains[i] + "." + key;
				domainCountMap.put(key, domainCountMap.getOrDefault(key, 0) + count);
			}
		}

		for (String key : domainCountMap.keySet())
			visitedCounts.add(domainCountMap.get(key) + " " + key);

		return visitedCounts;
	}

	public static void main(String[] args) {
		String[] domains = { "9001 discussion.leetcode.com" };
