...
/*
 Problem : Given an Integer (positive or negative), write a function which returns the squareroot
 */

public class Sqrt {

	/**
	 * Solution 1 : Brute-Force 1. Start with initial value of i as 1 and result
	 * 1. 2. calculate the result ( i * i) until the result is <= number runtime
	 * : O(N), space : O(1);
	 */
	...
	/**
	 * Solution 2 : efficient (binary search) 1. have start - 0 and end = number.
	 * 2. calculate the mid = start + (end-start)/2 3. divide the number of
	 * calculation based on left or right. 4. Note : Take the min of (start and
	 * end) when not a perfect square. runtime : O(log n), space : O(1);
	 */
	private static int sqrtV2(int x) {
		int start = 1;
		int end = x;
		while (start <= end) {
	...
	/*
	 * The following uses a precision to a given value 
	...
	public static void main(String[] args) {
		int number = 17;
		System.out.println("square root of number " + number + " is = " + sqrtV1(number));
		System.out.println("square root of number " + number + " is = " + sqrtV2(number));
